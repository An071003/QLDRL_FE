...
export default function AdvisorClassesPage() {
  ...
  const paginatedClasses = sortedAndFilteredClasses.slice(
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                >
                  Tên lớp {sortField === 'name' && (sortDirection === 'asc' ? '▲' : '▼')}
                </th>
                <th 
                  className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase cursor-pointer"
                  onClick={() => handleSort('cohort')}
                >
                  Khóa {sortField === 'cohort' && (sortDirection === 'asc' ? '▲' : '▼')}
                </th>
                <th 
                  className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase cursor-pointer"
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {paginatedClasses.map((classItem) => (
                <tr key={classItem.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3 whitespace-nowrap">{classItem.id}</td>
                  <td className="px-4 py-3 whitespace-nowrap">
                    <div className="max-w-xs truncate" title={classItem.name}>{classItem.name}</div>
                  </td>
              ...
              <div>
                <p className="text-sm text-gray-700">
                  ...
                  <span className="font-medium">
                    {Math.min(currentPage * itemsPerPage, sortedAndFilteredClasses.length)}
                  </span>{' '}
                  trong tổng số <span className="font-medium">{sortedAndFilteredClasses.length}</span> lớp
                </p>
              </div>
              <div>
